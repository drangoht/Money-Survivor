...
public class Program {
    public static void ProcessImage(string path) {
        try {
            ...
            // Assume top-left corner is the background color we want to replace
            Color bgKey = bmp.GetPixel(5, 5);
            float thresh = 40f; 

            // Simple flood fill to find contiguous background
            ...
            while(q.Count > 0) {
                ...
                // If it's pure transparent (from my last script) OR matches the corner color
                bool isBg = c.A < 10 || 
