...
public class ProgramBgFixer {
    public static void ProcessImage(string path, Color targetBgColor) {
        try {
            ...
            float thresh = 80f; // High threshold since we filled it with pure magenta/white before
            ...
            while(q.Count > 0) {
                ...
                // Check if it's the old background (magenta, empty, or white from previous scripts)
                bool isBg = c.A < 10 || 
                           (Math.Abs(c.R - bgKey.R) + Math.Abs(c.G - bgKey.G) + Math.Abs(c.B - bgKey.B) < thresh) ||
                           (c.R > 230 && c.G < 50 && c.B > 230); // Check for magenta
